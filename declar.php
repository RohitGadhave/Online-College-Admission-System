...
$count_UID=count($UID_from_jeemarks);
 ...
 for ($i=0 ; $i < $count_UID ; $i++)
  { 
	 ...
	 $sql = "SELECT college1,college2,college3 FROM collegepref1 WHERE UID = $UID_from_jeemarks[$i]";
...
echo "START   ";
	...
	if ( $rowc1['vacc'] != 0 && $JEE_from_jeemarks[$i]>=$rowc1['cut_off'] ) 
	{
		...
		$sql_r="INSERT INTO result1(UID,college_name) VALUES ($UID_from_jeemarks[$i],'$college1' )";
	...
	else if( $rowc2['vacc'] != 0 && $JEE_from_jeemarks[$i]>=$rowc2['cut_off'] )
	{
		...
		$sql_r="INSERT INTO result1(UID,college_name) values ($UID_from_jeemarks[$i],'$college2' )";
		 $sql_run = mysqli_query($con,$sql_r) or die(mysqli_error($con));
	...
	else if( $rowc3['vacc'] != 0 && $JEE_from_jeemarks[$count_UI]>=$rowc3['cut_off'] )
	{
		...
		$sql_r="INSERT INTO result1(UID,college_name) values ($UID_from_jeemarks[$i],'$college3' )";
		$sql_run = mysqli_query($con,$sql_r) or die(mysqli_error($con));
